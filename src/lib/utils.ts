...
export async function parseCommandLineArgs(args: string[], usage: string) {
  ...
  // Parse token type
  let tokenType: 'access_token' | 'id_token' = 'access_token'
  const tokenTypeIndex = args.indexOf('--token-type')
  if (tokenTypeIndex !== -1 && tokenTypeIndex < args.length - 1) {
    const tt = args[tokenTypeIndex + 1]
    if (tt === 'access_token' || tt === 'id_token') {
      tokenType = tt
      log(`Using token type: ${tokenType}`)
    } else {
      log(`Warning: Ignoring invalid token type: ${tt}. Valid values are: access_token, id_token`)
    }
  }
