...
export class NodeOAuthClientProvider implements OAuthClientProvider {
  ...
  private tokenType: 'access_token' | 'id_token'
  ...
  constructor(readonly options: OAuthProviderOptions) {
    ...
    this.tokenType = options.tokenType || 'access_token'
  }
  ...
  async tokens(): Promise<OAuthTokens | undefined> {
    ...
    if (tokens) {
      ...
      // If configured to use id_token, swap it into access_token so the SDK sends it as Bearer
      if (this.tokenType === 'id_token' && tokens.id_token) {
        debugLog('Using id_token as Bearer token')
